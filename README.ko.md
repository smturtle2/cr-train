...
build_sen12mscr_loaders()          ← scene 단위 분할, 스트리밍 디코딩, 전처리
  │
  ├── train_loader                 ← 파일 shard 스트리밍 + sample-buffer shuffle
  ├── val_loader                   ← 고정 순서
  └── test_loader                  ← 고정 순서
        │
        ▼
...
**데이터 파이프라인.** Parquet shard를 Hugging Face에서 직접 스트리밍합니다 -- 로컬 다운로드가 필요 없습니다. 필요한 parquet 컬럼만 읽은 뒤 각 sample을 CHW 텐서(2x256x256 SAR, 13x256x256 optical)로 디코딩하고 즉시 정규화합니다.
...
├── src/cr_train/
│   ├── __init__.py         # 공개 API
│   ├── _parquet_streaming.py  # parquet column pruning과 scan 튜닝
