...
{
    "inputs": {
        "sar":    Tensor,  # [B, 2, 256, 256]   SAR 후방산란, float32 [0, 1]
        "cloudy": Tensor,  # [B, 13, 256, 256]  구름 낀 Sentinel-2, float32 [0, 1]
    },
...
Trainer는 `inputs`의 값을 위치 인자로 풀어서 `model(sar, cloudy)` 형태로 호출합니다. 순서는 항상 SAR, cloudy 순이며 `forward()`의 파라미터 이름은 자유입니다.
