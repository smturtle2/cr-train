...
| `scheduler` | `LRScheduler \| None` | `None` | 같은 optimizer로 생성한 optional epoch scheduler. `Trainer.step()`이 validation 뒤에 `scheduler.step()`을 한 번 호출합니다. `ReduceLROnPlateau`는 지원하지 않습니다. |
