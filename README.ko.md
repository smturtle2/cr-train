...
> Hugging Face rate limit가 적용됩니다. `export HF_TOKEN=your_token`을 설정하면 더 빠릅니다. 런타임에서는 `from cr_train import hf_token_status`로 인증 상태와 적용 여부를 확인할 수 있습니다.
...
| `persistent_workers` | `bool \| None` | `None` | 기본은 자동 `False`; epoch 사이 worker 재사용이 꼭 필요할 때만 명시적으로 켬 |
