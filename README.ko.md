...
trainer = Trainer(
    ...
    batch_size=4,
    accum_steps=4,  # optimizer step마다 4개 micro-batch 누적
    epochs=2,
    seed=42,
...
train batch는 active cached block들 사이에서 결정적으로 sample 단위로 섞이고, validation/test는 선택된 block 순서를 유지합니다.
`--grad-clip-norm VALUE`를 주면 optimizer update 전에 gradient norm clipping을 적용합니다. train loss나 gradient가 non-finite이면 `optimizer.step()` 전에 즉시 실패합니다.
...
증강은 train split에만 적용되고, validation/test는 원본 `256x256` 크기를 유지합니다.
...
- iterable DataLoader, rank-aware block 분할, sample-mixed train order 구성
- `metrics.jsonl` 기록
- 학습 중 배치 단위 tqdm으로 running-average loss와 메트릭을 표시하고, split별 소요 시간이 포함된 정렬된 train/validation 요약 줄을 출력
- 출력 디렉토리의 `metrics.jsonl`에 줄 단위 JSON 객체로 메트릭 기록

일반적인 학습 흐름에서는 캐시나 데이터로더 헬퍼를 직접 호출할 필요가 없습니다.
저장/복원과 추론은 명시적으로 호출합니다. 필요할 때 `save_checkpoint()`, `load_checkpoint()`, `save_weights()`, `load_weights()`, `predict()`, `get_state()`를 사용하세요.
...
| `scheduler` | `LRScheduler \| None` | `None` | 같은 optimizer로 생성한 optional scheduler. 일반 scheduler는 validation 뒤에 한 번 step하고, `ReduceLROnPlateau`도 지원합니다. |
| `scheduler_timing` | `str` | `"after_validation"` | `Trainer`가 `scheduler.step()`을 호출하는 시점입니다. 지원 값은 `after_validation`, `before_optimizer_step`, `after_optimizer_step`입니다. 번들된 warmup-cosine 같은 epoch 기반 scheduler는 기본 `after_validation`을 유지하세요. |
| `scheduler_monitor` | `str \| None` | `None` | `ReduceLROnPlateau`에서 사용할 monitor 경로. 기본값은 `val.loss`이며 `val.loss`, `val.metrics.<name>`만 지원합니다. |
...
| `accum_steps` | `int` | `1` | optimizer step 전에 누적할 micro-batch 수. `global_step`은 micro-batch 수가 아니라 optimizer update 횟수를 셉니다. |
...
`model`, `optimizer`, `epoch`, `global_step`를 포함한 재개용 체크포인트를 저장합니다.
`global_step`은 개별 forward/backward micro-batch가 아니라 완료된 optimizer update 횟수를 셉니다.
scheduler를 설정했다면 `scheduler` 상태도 함께 저장합니다.
`path`를 생략하면 현재 완료된 epoch 기준으로 `<output_dir>/epoch-XXXX.pt`에 저장합니다.

### `Trainer.load_checkpoint(path: str | Path) -> dict`

체크포인트 파일에서 `model`, `optimizer`, `epoch`, `global_step`를 복원하고 다음 정보를 반환합니다:
gradient accumulation을 사용한 경우에도 복원된 `global_step`은 완료된 optimizer update 횟수를 뜻합니다.
Trainer와 체크포인트 양쪽에 scheduler 상태가 있으면 그것도 함께 복원합니다.
예전 체크포인트처럼 `scheduler` 키가 없어도 그대로 불러올 수 있지만, 그 경우 scheduler 진행 상태는 자동으로 재구성되지 않습니다.
...
모델 가중치만 저장합니다. `path`를 생략하면
`<output_dir>/model-epoch-XXXX.pt`에 저장합니다.

### `Trainer.load_weights(path: str | Path, *, strict: bool = True) -> None`

optimizer 상태, scheduler 상태, runtime counter는 건드리지 않고 모델 가중치만 복원합니다.
`save_weights()` 파일과 `save_checkpoint()` 파일 둘 다 입력으로 받을 수 있습니다.

### `Trainer.predict(batch: Mapping[str, Any]) -> Any`

한 배치에 대해 `eval()` + `no_grad()` 추론을 수행한 뒤, 호출 전 학습 모드를 복원합니다.
`batch`에는 최소한 `sar`, `cloudy` 텐서가 있어야 합니다.

### `Trainer.get_state() -> dict`

현재 runtime 상태를 반환합니다.
`global_step`은 완료된 optimizer update 횟수입니다.
