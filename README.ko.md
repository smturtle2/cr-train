...
# 1. 모델 정의 -- forward(sar, cloudy)만 구현하면 됩니다
...
**데이터 파이프라인.** Parquet shard를 Hugging Face에서 직접 스트리밍합니다 -- 로컬 다운로드가 필요 없습니다. 각 sample은 CHW 텐서로 디코딩되고 즉시 정규화됩니다.
...
**체크포인팅.** `checkpoint_dir`을 설정하면 매 epoch 후 `last.pt`와 `epoch-NNNN.pt`가 자동 저장됩니다. 체크포인트에는 전체 RNG 상태(Python, NumPy, Torch, CUDA)가 포함되어 정확한 재개가 가능합니다.
...
{
    "inputs": {
        "sar":    Tensor,  # [B, 2, H, W]   SAR 후방산란, float32 [0, 1]
        "cloudy": Tensor,  # [B, 13, H, W]  구름 낀 Sentinel-2, float32 [0, 1]
    },
    "target": Tensor,      # [B, 13, H, W]  구름 없는 Sentinel-2, float32 [0, 1]
    "metadata": {
...
`inputs`가 `dict`이므로 trainer는 `model(sar=..., cloudy=...)`로 호출합니다. list/tuple이면 위치 인자로, 단일 텐서면 그대로 전달됩니다.
...
`(train_loader, val_loader, test_loader)`를 반환합니다.
...
| `persistent_workers` | `bool \| None` | `None` | 자동 `False`; `True`로 설정하면 epoch 간 worker 유지 |
...
모델 파라미터에서 device를 자동 추론합니다. 배치 데이터는 해당 device로 자동 이동됩니다.
...
선택적 override: `max_epochs`, `train_max_batches`, `val_max_batches`.
...
`checkpoint_dir/filename`에 저장합니다. 체크포인팅이 비활성화되어 있으면 `None` 반환. 포함 내용:
모델 state, optimizer state, `TrainerState` (epoch, global_step), 전체 RNG 상태.
...
`trainer.state`로 접근 가능합니다. `epoch` (내부적으로 0부터 시작)과 `global_step` (누적 optimizer step 수)을 추적합니다. 체크포인트에 함께 저장됩니다.
...
| Epoch별 셔플 | `set_epoch()` 자동 전파 |
| 정확한 재개 | 체크포인트에 Python, NumPy, Torch, CUDA RNG 상태 캡처 |
...
cr-train/
├── src/cr_train/
│   ├── __init__.py         # 공개 API: Trainer, TrainerConfig, MAE, build_sen12mscr_loaders
