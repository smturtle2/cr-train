...
build_sen12mscr_loaders()          ← scene 단위 분할, 스트리밍 디코딩, 전처리
  │
  ├── train_loader                 ← 시작 시 row-group 확장 + sample-buffer shuffle
  ├── val_loader                   ← 고정 순서
  └── test_loader                  ← 고정 순서
        │
        ▼
...
**데이터 파이프라인.** Parquet shard를 Hugging Face에서 직접 스트리밍합니다 -- 로컬 다운로드가 필요 없습니다. train row group은 loader 생성 시 한 번만 펼치고, 필요한 parquet 컬럼만 읽은 뒤 각 sample을 CHW 텐서(2x256x256 SAR, 13x256x256 optical)로 디코딩하고 즉시 정규화합니다.

**Scene 격리.** 셔플링 전에 scene을 train/val/test에 배정합니다. 하나의 scene이 여러 split에 나타나지 않습니다.

**결정론적 순서.** 같은 `seed`를 사용하면 배치 순서가 완전히 재현됩니다. Trainer는 매 epoch마다 `set_epoch()`을 호출하여 train sample-buffer shuffle 순서만 결정론적으로 바꿉니다.
...
| `fragment_scan_options` | `ParquetFragmentScanOptions \| None` | `None` | pyarrow fragment cache/range 튜닝 override |
...
├── src/cr_train/
│   ├── __init__.py         # 공개 API
│   ├── _parquet_streaming.py  # parquet column pruning과 row-group 확장
