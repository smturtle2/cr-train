...
PyTorch 모델만 가져오세요 -- 스트리밍 데이터 로딩, 전처리, 체크포인팅, 진행 표시는 cr-train이 처리합니다.
...
> Hugging Face rate limit가 적용됩니다. `export HF_TOKEN=your_token`을 설정하면 더 빠릅니다. `build_loaders()`는 가능하면 `huggingface_hub.get_token()`이 찾은 cached token을 전달합니다.
...
# 2. 데이터 로더 생성 (Hugging Face에서 스트리밍)
...
더 많은 예제: [`examples/minimal_train.py`](./examples/minimal_train.py) (CLI 예제), [`examples/colab_quickstart.ipynb`](./examples/colab_quickstart.ipynb) (Colab 노트북).
...
Hugging Face (parquet shard)
  │
  ▼
build_loaders()                    ← HF split 로드, 디코딩, 전처리
  │
  ├── train_loader                 ← seed 기반 sample-buffer shuffle
  ├── val_loader                   ← 고정 순서
  └── test_loader                  ← 고정 순서
        │
        ▼
...
**데이터 파이프라인.** `build_loaders()`는 `Hermanni/sen12mscr`의 Hugging Face `train`, `validation`, `test` split을 직접 스트리밍합니다. 필요한 parquet 컬럼만 읽고, 각 sample을 CHW 텐서(2x256x256 SAR, 13x256x256 optical)로 디코딩한 뒤 즉시 정규화합니다. `train`만 `seed`와 `shuffle_buffer_size`를 사용해 셔플되고, `val/test`는 split 순서를 유지합니다.

**결정론적 순서.** 같은 `seed`를 사용하면 train split이 같은 Hugging Face streaming shuffle 순서를 사용합니다. `Trainer`는 train loader에만 `set_epoch()`를 호출하므로 validation/test는 고정 순서를 유지합니다.
...
`inputs`는 `(sar, cloudy)` tuple입니다. Trainer는 이를 `model(sar, cloudy)`로 풀어서 호출합니다. `forward()`의 파라미터 이름은 자유입니다.
...
| `dataset` | `str` | `"Hermanni/sen12mscr"` | `datasets.load_dataset()`에 넘길 Hugging Face 데이터셋 이름 |
| `seed` | `int` | `0` | train shuffle 순서 시드 |
| `shuffle_buffer_size` | `int` | `64` | 학습용 메모리 내 sample shuffle 버퍼 크기 |
| `num_workers` | `int` | `0` | PyTorch dataloader worker 수 |
...
| `train_max_samples` | `int \| None` | `None` | epoch당 학습 샘플 수 상한 |
| `val_max_samples` | `int \| None` | `None` | epoch당 검증 샘플 수 상한 |
| `test_max_samples` | `int \| None` | `None` | 테스트 샘플 수 상한 |
...
`scheduler`가 설정되어 있으면 매 epoch 후 `scheduler.step()`이 호출됩니다. 선택적 override: `max_epochs`, `train_max_samples`, `val_max_samples`.

#### `trainer.test(**overrides) -> dict[str, float]`

`{"loss": ..., "mae": ...}`를 반환합니다. 선택적 override: `max_samples`.
...
| 보장 사항 | 메커니즘 |
|-----------|----------|
| 결정론적 train 셔플 | 같은 `seed` + 같은 설정 = 동일한 HF streaming shuffle 순서 |
| 고정된 val/test 순서 | validation/test loader는 셔플하지 않음 |
| Train 전용 epoch hook | `Trainer`는 train loader에만 `set_epoch()`를 호출함 |
...
│   ├── data.py             # HF streaming loader, 배치 디코딩, 전처리
├── examples/
│   ├── minimal_train.py    # 최소 CLI 학습 스크립트
│   └── colab_quickstart.ipynb
├── tests/
├── scripts/
│   └── refresh_official_scene_splits.py  # 저장소 유지보수용 유틸리티
└── pyproject.toml
```
