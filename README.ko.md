...
PyTorch 모델만 가져오세요 -- 결정론적 데이터 로딩(스트리밍 또는 전체 다운로드), 전처리, 체크포인팅, 진행 표시는 cr-train이 처리합니다.
...
# 2. 데이터 로더 생성 (기본: Hugging Face에서 스트리밍)
...
Hugging Face (parquet shard)
  │
  ▼
build_sen12mscr_loaders()          ← scene 단위 분할, 디코딩, 전처리
  │
  ├── train_loader                 ← 스트리밍: sample-buffer shuffle / 비스트리밍: sampler shuffle
  ├── val_loader                   ← 고정 순서
  └── test_loader                  ← 고정 순서
        │
        ▼
...
**결정론적 순서.** 같은 `seed`를 사용하면 배치 순서가 완전히 재현됩니다. Trainer는 매 epoch마다 `set_epoch()`을 호출하여 train sample-buffer shuffle 순서만 결정론적으로 바꿉니다. 파일 shard 순서는 고정됩니다.
...
| `persistent_workers` | `bool \| None` | `None` | workers > 0일 때 자동 `True` |
| `io_profile` | `str` | `"smooth"` | `"smooth"` (throughput 우선 readahead) 또는 `"conservative"` (완전 동기식) |
| `cache_options` | `CacheOptions \| None` | `None` | 선택적 pyarrow range-cache override; 생략하면 `"smooth"`에서 기본 cache preset 사용 |
...
| Epoch별 셔플 | `set_epoch()`가 train sample buffer만 다시 시드함 |
...
├── src/cr_train/
│   ├── __init__.py         # 공개 API
│   ├── _parquet_streaming.py  # parquet 로딩 (스트리밍 + map-style)
│   ├── trainer.py          # 학습 루프, 체크포인팅, 진행 표시
│   ├── data.py             # 데이터셋 (스트리밍 + map-style), scene 분할, 전처리
